...
#define RCOMPONENT "OMX.Nvidia.mp4.read"
#define DCOMPONENT "OMX.Nvidia.h264ext.decode"
...
static OMX_HANDLETYPE decoderhandle, readerhandle;
...
static OMX_ERRORTYPE decoderEventHandler(
	...
	case OMX_EventPortSettingsChanged:
		if(Data2 != 1) {
			fprintf(stderr, "got a INPUT port changed event ! %d\n", Data2);
		}
		if(hComponent == decoderhandle)
			printf("decoder\n");

		if(hComponent == readerhandle)
			printf("reader\n");

		OMX_ERRORTYPE err;
		OMX_PARAM_PORTDEFINITIONTYPE paramPort;
		setHeader(&paramPort, sizeof(paramPort));
		paramPort.nPortIndex = Data2;

		err=OMX_GetParameter(hComponent, OMX_IndexParamPortDefinition, &paramPort);
		OMXE(err);
...
static int init()
{
	...
	err = OMX_GetHandle(&decoderhandle, DCOMPONENT, NULL, &decodercallbacks);
	OMXE(err);

	err = OMX_GetHandle(&readerhandle, RCOMPONENT, NULL, &decodercallbacks);
	OMXE(err);
	...
	err=OMX_GetExtensionIndex(readerhandle, FILENAME, &filename_idx);
	OMXE(err);
	printf("filename: %d\n");
	...
	err=OMX_SetParameter(readerhandle, filename_idx, &filename);
	OMXE(err);

	err = OMX_SendCommand(readerhandle, OMX_CommandPortDisable, 1, 0);
	OMXE(err);
	...
	err=OMX_GetParameter(readerhandle, OMX_IndexParamPortDefinition, &paramPort);
	OMXE(err);
	...
	*/
	...
	buffer_out_size=paramPort.nBufferSize;
    	...
    	err = OMX_SetupTunnel(readerhandle, 0, decoderhandle, 0);
	OMXE(err);
